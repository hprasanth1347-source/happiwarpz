  ...
  try {
    if (isDatabaseConnected) {
      try {
        ...
        logger.warn("Admin customers lookup fallback:", e.message);
      }
    }
  ...
  try {
    ...
    if (isDatabaseConnected) {
      try {
        ...
        return sendSuccess(res, `Customer status updated to ${newStatus}.`, { customer: updatedCustomer });
      } catch (e) {}
    }

    return sendSuccess(res, `Customer status updated to ${newStatus}.`, {
