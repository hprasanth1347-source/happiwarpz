...
function assert(condition, testName, details) {
  ...
  if (condition) {
    ...
    testResults.push({ name: testName, status: 'PASS', details: details || '' });
  } else {
    failCount++;
    console.error('  ❌ [FAIL] ' + testName + (details ? ' - ' + details : ''));
    testResults.push({ name: testName, status: 'FAIL', details: details || '' });
  }
}
...
async function runAuditSuite() {
  ...
  try {
    ...
    assert(health.status === 200 && health.data && health.data.success === true, 'GET /api/health returns 200 healthy status');
    assert(health.data && health.data.data && health.data.data.status === 'healthy', 'Health check reports status: "healthy"');
    ...
    assert(regRes.status === 201 && regRes.data && regRes.data.success === true, 'POST /api/auth/register creates user account');
    assert(regRes.data && regRes.data.data && Boolean(regRes.data.data.token), 'Registration returns valid JWT authentication token');

    const customerToken = regRes.data && regRes.data.data ? regRes.data.data.token : '';
    ...
    assert(badReg.status === 400 && badReg.data && badReg.data.success === false, 'Registration rejects missing required fields (400)');
    ...
    assert(loginRes.status === 200 && loginRes.data && loginRes.data.success === true, 'POST /api/auth/login succeeds with valid credentials');
    ...
    assert(adminLoginRes.status === 200 && adminLoginRes.data && adminLoginRes.data.success === true, 'POST /api/auth/admin-login succeeds for admin');
    assert(adminLoginRes.data && adminLoginRes.data.data && adminLoginRes.data.data.user && adminLoginRes.data.data.user.role === 'ADMIN', 'Admin login returns ADMIN role claims');

    const adminToken = adminLoginRes.data && adminLoginRes.data.data ? adminLoginRes.data.data.token : '';
    ...
    assert(badAdminLogin.status === 401 && badAdminLogin.data && badAdminLogin.data.success === false, 'Admin login rejects invalid credentials (401)');
    ...
    assert(googleLoginRes.status === 200 && googleLoginRes.data && googleLoginRes.data.success === true, 'POST /api/auth/google generates valid session');

    // Current User Profile (/api/auth/me)
    const meCustomer = await request('GET', '/api/auth/me', null, customerAuthHeader);
    assert(meCustomer.status === 200 && meCustomer.data && meCustomer.data.data && meCustomer.data.data.user && meCustomer.data.data.user.email === testEmail, 'GET /api/auth/me returns authenticated customer details');

    const meAdmin = await request('GET', '/api/auth/me', null, adminAuthHeader);
    assert(meAdmin.status === 200 && meAdmin.data && meAdmin.data.data && meAdmin.data.data.user && meAdmin.data.data.user.role === 'ADMIN', 'GET /api/auth/me returns authenticated admin details');
    ...
    assert(logoutRes.status === 200 && logoutRes.data && logoutRes.data.success === true, 'POST /api/auth/logout clears auth session');
    ...
    // Customer token attempting admin route
    ...
    assert(categoriesRes.status === 200 && categoriesRes.data && categoriesRes.data.data && categoriesRes.data.data.categories && categoriesRes.data.data.categories.length > 0, 'GET /api/categories returns product categories');

    const productsRes = await request('GET', '/api/products');
    assert(productsRes.status === 200 && productsRes.data && productsRes.data.data && productsRes.data.data.products && productsRes.data.data.products.length > 0, 'GET /api/products returns catalog products');

    const productList = productsRes.data && productsRes.data.data ? productsRes.data.data.products : [];
    const sampleProduct = productList[0] || { id: 'rose-bouquet-without-glitter', slug: 'rose-bouquet-without-glitter', name: 'Rose Bouquet' };
    const sampleProductId = sampleProduct.id;
    const sampleProductSlug = sampleProduct.slug;

    const roseProduct = await request('GET', '/api/products/' + sampleProductSlug);
    ...
    assert(addToCartRes.status === 200 && addToCartRes.data && addToCartRes.data.success === true, 'POST /api/cart adds customized product to user cart');

    // Get Cart
    const getCartRes = await request('GET', '/api/cart', null, customerAuthHeader);
    assert(getCartRes.status === 200 && getCartRes.data && getCartRes.data.success === true, 'GET /api/cart retrieves cart items and subtotal');

    // Wishlist Toggle (Add)
    const wishAdd = await request('POST', '/api/wishlist', { productId: sampleProductId }, customerAuthHeader);
    ...
    assert(wishGet.status === 200 && wishGet.data && wishGet.data.success === true, 'GET /api/wishlist retrieves user wishlist');
    ...
    const placeOrderRes = await request('POST', '/api/orders', {
      ...
      cartItems: [
        {
          productId: sampleProductId,
          productName: sampleProduct.name || 'Handmade Flower Bouquet',
          quantity: 1,
          price: sampleProduct.price || 999,
          variant: 'Standard',
    ...
    assert(placeOrderRes.status === 201 && placeOrderRes.data && placeOrderRes.data.success === true, 'POST /api/orders places customer order');

    const createdOrder = placeOrderRes.data && placeOrderRes.data.data ? placeOrderRes.data.data.order : null;
    const orderId = createdOrder ? createdOrder.id : 'ord_101';

    // Get User Orders
    const userOrdersRes = await request('GET', '/api/orders', null, customerAuthHeader);
    assert(userOrdersRes.status === 200 && userOrdersRes.data && userOrdersRes.data.success === true, 'GET /api/orders retrieves user order history');

    // Order chat: unauthenticated rejected
    const unauthChat = await request('GET', '/api/orders/' + orderId + '/messages');
    ...
    assert(payOrderRes.status === 200 && payOrderRes.data && payOrderRes.data.success === true, 'POST /api/payments/create-order initializes payment order');
    ...
    assert(verifyPayRes.status === 200 && verifyPayRes.data && verifyPayRes.data.success === true, 'POST /api/payments/verify validates test payment token');
    ...
    const customReqRes = await request('POST', '/api/custom-requests', {
      ...
      productType: 'Anniversary Party',
      budget: 4500,
      preferredColors: 'Pink, Lavender & Gold',
      specialInstructions: 'Looking for 10 customized flower arrangements with personalized gift tags.',
    });
    assert(customReqRes.status === 201 && customReqRes.data && customReqRes.data.success === true, 'POST /api/custom-requests submits inquiry');
    ...
    const getReviewsRes = await request('GET', '/api/reviews/product/' + sampleProductId);
    assert(getReviewsRes.status === 200 && getReviewsRes.data && getReviewsRes.data.success === true, 'GET /api/reviews/product/:id returns reviews list');
    ...
    assert(sqliSearch.status === 200 && sqliSearch.data && sqliSearch.data.success === true, 'SQL injection strings in search do not crash or leak internals');
    ...
    assert(malformedReq.status === 400 && malformedReq.data && malformedReq.data.error === 'INVALID_JSON', 'Malformed JSON payload returns clean 400 INVALID_JSON');

    // Unknown 404 Route handling
    const notFoundRes = await request('GET', '/api/unknown-non-existent-endpoint');
    assert(notFoundRes.status === 404 && notFoundRes.data && notFoundRes.data.error === 'NOT_FOUND', 'Unknown endpoint returns standardized 404 error');
  ...
  if (failCount === 0) {
    ...
    console.error('❌ ' + failCount + ' TESTS FAILED. PLEASE REVIEW LOGS ABOVE.');
    process.exit(1);
  }
}
